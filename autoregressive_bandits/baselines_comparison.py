...
if __name__ == '__main__':
    ...
    out_folder = f'autoregressive_bandits/output/simulation_{simulation_id}/'
    try:
        os.mkdir(out_folder)
    ...
    for testcase_id in sys.argv[2:]:
        ...
        f = open(f'autoregressive_bandits/input/testcase_{testcase_id}.json')
        ...
        core = Core(env, agent)
        logs['EXP3'], a_hists['EXP3'] = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)
        logs['EXP3'] = logs['EXP3'][:, len(param_dict['X0']):]
        ...
        core = Core(env, agent)
        logs['MiniBatchEXP3'], a_hists['MiniBatchEXP3'] = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)
        ...
        core = Core(env, agent)
        logs['AR2'], a_hists['AR2'] = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)
        logs['AR2'] = logs['AR2'][:, len(param_dict['X0']):]
        ...
        core = Core(env, agent)
        logs['ARB'], a_hists['ARB'] = core.simulation(n_epochs=param_dict['n_epochs'], n_rounds=T)
        logs['ARB'] = logs['ARB'][:, len(param_dict['X0']):]
        ...
        x[-1] = min(x[-1],len(np.mean(np.cumsum(regret['ARB'].T, axis=0), axis=1))-1)
        f,ax = plt.subplots(1, figsize=(20,10))
        sqrtn = np.sqrt(param_dict['n_epochs'])

        for i,label in enumerate(regret.keys()):
            ax.plot(x, np.mean(np.cumsum(regret[label].T, axis=0), axis=1)[x], label=label, color=f'C{i+1}')
